...
module.exports = (function () {
	...
	function ensureInitialize (self, callback) {
		...
		return initialize(self, function (err) {
			if (err) {
				return callback(err);
			}
		});
	...
	function initialize(self, callback) {
		...
		return client.indices.exists(function (err, info) {
			...
			if (!info.exists) {
				...
				return client.indices.createIndex(data, callback);
			}

			// call to put mapping because the index exists
			var data = {};
			data[self._options._type] = self._mapping;
			return client.indicies.putMapping(data, callback);
		});
	}
	...
	function update (self, _id, doc, upsert, callback) {
		...
		return self.validate(doc, function (err, model, modelId) {
			if (err) {
				return callback(err);
			}
			...
			return ensureInitialize(self, function (err) {
				...
				return self._client.update(
					...
					function (err, result) {
						...
						// TODO: integration test this to verify return

						// ensure type values are as expected
						coerceProperties(self, model);
