...
module.exports = (function () {
	...
	Mapper.prototype.get = function (_id, callback) {
		...
		_this._client.get({
			...
			if (result) {
				return _this.parse(result, callback);
			}

			// return with nothing if there is no result
			return callback();
		});
	};
	...
	Mapper.prototype.validate = function (model, callback) {
		...
		if (Array.isArray(result.errors) && result.errors.length) {
			var err = new errors.InvalidMappingError(result.errors[0]);
	...
	/*
	Mapper.prototype.index = function (model, callback) {
		this.validate(model, function (err, result) {
			if (err) {
				return callback(err);
			}
