...
const
	...
	DEFAULT_NOT_FOUND = 404,
	DEFAULT_SCROLL_DURATION = '30s',
	EVENT_IDENTITY = 'identity',
	EVENT_SUMMARY = 'summary',
	EVENT_VERSION = 'version',
	validTypes = Object.keys(validators);
...
function prepareMappingForElasticsearch (mapping) {
	let preparedMapping = JSON.parse(JSON.stringify(mapping));

	stripDecorations(preparedMapping);

	return preparedMapping;
}

function scrollAndDelete (self, query, scrollId, state, callback) {
	...
	// exit immediately if bulk removal is complete
	if (state.total > 0 && state.count === state.total) {
		return setImmediate(callback);
	}

	return self._client.scroll(
		{
			scroll : DEFAULT_SCROLL_DURATION
		},
		scrollId,
		(err, result) => {
			if (err) {
				err.desc = '#delete - unable to scan results using scroll identifier';
				err.scrollId = scrollId;
			...
			if (result && result.hits && result.hits.hits) {
				// ensure state
				state.count += result.hits.hits.length;
				state.total = state.total || result.hits.total;

				// build bulk delete commands
			...
			// call bulk if there are results to remove
			if (commands && commands.length) {
				// perform bulk delete operation
				return self._client.bulk(commands, (err) => {
					if (err) {
						err.desc = '#delete - unable to delete by query';
					...
					// continue processing
					return scrollAndDelete(self, query, scrollId, state, callback);
				});
			}
...
function stripDecorations (properties) {
	if (properties.properties) {
		return stripDecorations(properties.properties);
	}

	Object.keys(properties).forEach((field) => {
		if (properties[field].properties) {
			return stripDecorations(properties[field].properties);
		}

		DECORATED_SCHEMA_PARAMETERS.forEach((decoratedParameter) => {
			properties[field][decoratedParameter] = undefined;
		});
	});
}
...
function validateModel (self, model, parentPath, skipRequiredFieldCheck) {
	...
	Object.keys(model).some((field) => {
		...
		// determine the expected type (if specified)
		expectedType = self._fields[fieldPath] ?
			self._fields[fieldPath].type :
			undefined;
		...
		if (validators.isPOJO(model[field]) && expectedType !== 'object') {
			let fieldResult = validateModel(
				self,
				fieldValue,
				fieldPath,
...
export class Mapper extends EventEmitter {
	...
	bulkCreate (idList, docList, callback) {
				...
				return ensureInitialize(_this, (err) => {
					...
					return _this._client.bulk(commands, (err, result) => {
						...
						if (result && result.items) {
							result.items.some((item, i) => {
								if (item && item.index && item.index.error) {
									...
									// carry forward any additional details
									err.desc = item.index.error;
									err.statusCode = item.index.status;
						...
						if (!modelIds.length) {
							_this.emit(
								EVENT_IDENTITY,
								result.items.map((item) => item.index._id));
						}
	...
	bulkGet (idList, _source, callback) {
	...
	bulkUpdate (idList, docList, callback) {
		if (typeof callback === 'undefined' && typeof docList === 'function') {
			callback = docList;
			docList = idList;
			idList = [];
	...
	bulkUpsert (idList, docList, callback) {
		if (typeof callback === 'undefined' && typeof docList === 'function') {
			callback = docList;
			docList = idList;
			idList = [];
		...
		let
			...
			exec = new Promise((resolve, reject) => {
				let
					...
					if (validators.isPOJO(_id)) {
						options = _id;
						options['op_type'] = options['op_type'] || 'index';
					} else {
						// default options when not specified as POJO
						options = {
							_id,
				...
				return ensureInitialize(_this, (err) => {
					...
					if (options._id) {
						return _this._client.delete(options, (err, summary) => {
							if (err) {
								err.desc =
									'#delete - unable to delete specified document in Elasticsearch';
								err.options = options;
					...
					// ensure _source is not retrieved for efficiency
					options._source = false;

					// search for items matching query and create scroll identifier
					return _this._client.search(
						{
							scroll : DEFAULT_SCROLL_DURATION
						},
						options,
						(err, result) => {
							if (err) {
								err.desc =
									'#delete - unable to delete based on query in Elasticsearch';
								err.query = options;
							...
							// delete by query (using scroll)
							return scrollAndDelete(_this, options, scrollId, (err) => {
								if (err) {
									return reject(err);
								}
			...
			exec = new Promise((resolve, reject) => {
				...
				return ensureInitialize(_this, (err) => {
					...
					return _this._client.get(options, (err, result) => {
						...
						if (err && err.statusCode !== DEFAULT_NOT_FOUND) {
